...
class Detector(object):
    ...
    def detect(self, batch):
        ...
        for b in range(batch_size):
            ...
            if self.cfg.debug == 2:
                for file_name in os.listdir(self.cfg.debug_dir):
                    # construct full file path
                    file = self.cfg.debug_dir + file_name
                ...
                gt_class_ids, gt_boxes = self.dataset.load_annotations(image_meta['index'])
                visualize_boxes(image, det['class_ids'], det['boxes'], det['scores'],
                                class_names=self.cfg.class_names,
                                save_path=save_path,
                                show=False)
                # visualize_boxes(image, gt_class_ids, gt_boxes, class_names=self.dataset.class_names,
...
class DataWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image, image_meta, gt_boxes = self.dataset.preprocess(image, image_meta, gt_boxes)
