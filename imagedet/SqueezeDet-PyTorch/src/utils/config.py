...
class Config(object):
    def __init__(self):
        ...
        self.parser.add_argument('--external_config', default=None)
        ...
        self.parser.add_argument('--drift_prob', type=float, default=0.2,
        ...
        self.parser.add_argument('--seed', type=int, default=42,
                                 help='random seed')
        self.parser.add_argument('--sharing_method', default=None)
        self.parser.add_argument('--sharing_acceleration_factor',type=int, default=10)
    ...
    def parse(self, args=''):
        ...
        if cfg.external_config is None:
            configuration_file_input=readConfigurationFile(configuration_file_path)

        if cfg.mode=='train':
            if 'load_model_training' in configuration_file_input:
                if configuration_file_input['load_model_training'] is not None:
                    if os.path.exists(configuration_file_input['load_model_training']):
                        cfg.load_model=configuration_file_input['load_model_training']

        # cfg.data_dir = os.path.join(cfg.root_dir, 'data')
        cfg.data_dir=configuration_file_input['root']+configuration_file_input['data_root']+configuration_file_input['path_to_dataset']
