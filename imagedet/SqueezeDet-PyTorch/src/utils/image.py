...
def whiten(image, image_meta, mean=0., std=1.):
    ...
    """
    #TODO
    nmean=np.mean(image, axis=tuple(range(image.ndim-1)))
    nstd=np.std(image, axis=tuple(range(image.ndim-1)))
    # image = (image - mean) / std
    # image_meta.update({'rgb_mean': mean, 'rgb_std': std})

    image = (image - nmean) / nstd
    image_meta.update({'rgb_mean': nmean, 'rgb_std': nstd})
...
def drift(image, image_meta, prob=0., boxes=None):
    ...
    if np.random.uniform() < prob:
        #max_drift_y = image_meta['orig_size'][0] // 8
        #max_drift_x = image_meta['orig_size'][1] // 16
        max_drift_y=16
        max_drift_x=16
