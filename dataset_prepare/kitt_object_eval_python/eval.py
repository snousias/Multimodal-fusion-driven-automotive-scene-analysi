...
from kitt_object_eval_python.rotate_iou import rotate_iou_gpu_eval
...
@numba.jit
def get_thresholds(scores: np.ndarray, num_gt, num_sample_pts=11):
    scores.sort()
    scores = scores[::-1]
    current_recall = 0
    thresholds = []
...
def do_coco_style_eval(gt_annos,
    ...
    for i in range(overlap_ranges.shape[1]):
        for j in range(overlap_ranges.shape[2]):
            ...
            ov=overlap_ranges[:, i, j]
            min_overlaps[:, i, j] = np.linspace(ov[0],ov[1],num=int(ov[2]))
