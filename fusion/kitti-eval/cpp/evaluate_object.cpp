...
const double   MIN_OVERLAP[3] = {0.5, 0.5, 0.3};                  // the minimum overlap required for evaluation
...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  for (int32_t j=0; j<2; j++) {
    ...
    if (j==0) {
      fprintf(fp,"set term png size 450,315 font \"Arial\" 11\n");
      fprintf(fp,"set output \"%s.png\"\n",file_name.c_str());
    } else {
      fprintf(fp,"set term postscript eps enhanced color font \"Arial\" 20\n");
...
//  string result_dir     = "results/" + result_sha;
  ...
  bool compute_aos=true, eval_car=true, eval_pedestrian=true, eval_cyclist=true;
