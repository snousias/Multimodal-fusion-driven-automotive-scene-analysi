...
from fusion.imagedet.src.utils.misc import MetricLogger
...
class Detector(object):
    ...
    @torch.no_grad()
    def detect(self, batch,cutoff=None,doVisualize=False):
        image_meta = {k: v.cpu().numpy()[0] if not isinstance(v, list) else v for k, v in batch['image_meta'].items()}
        # output=batch['image'].detach().cpu().numpy()[0]
        # output = np.moveaxis(output, 0, -1)
        # output=image_postprocess(output,image_meta)
        # output=output.astype(np.uint8)

        dets = self.model(batch) # this funticn detects
        ...
        for b in range(batch_size):
            ...
            output = None
            if doVisualize:
                output=visualize_boxes(image, det['class_ids'], det['boxes'], det['scores'],
                                class_names=self.cfg.class_names,
                                save_path=None,cutoff=cutoff)
            else:
                output=None
